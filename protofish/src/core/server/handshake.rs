...
pub async fn server_handshake<S: UTPStream>(pmc: &PMC<S>) -> Result<(), ProtofishError> {
    let (client_hello, ctx) = get_client_hello(pmc).await?;

    if let Payload::ClientHello(client_hello) = client_hello {
        ...
        Err(ConnectionError::MalformedPayload("expected ClientHello".into(), client_hello).into())
    }
}
...
async fn get_client_hello<S: UTPStream>(
    pmc: &PMC<S>,
) -> Result<(Payload, Context<S>), ProtofishError> {
    pmc.next_context()
        .await
